 ...
 */
public class GainsDemo2 extends ControlSystemMFN150
	{
	...
	private ParamBox2 pb;
	private boolean vecsum = true;
	private boolean lastvecsum = true;
	...
	public void configure()
		...
		// AS_GO_HOME
		//======
		NodeVec2 AS_GO_HOME = null;

		if (vecsum == false)
			{
			AS_GO_HOME = new v_WinnerTakeAll_va();

			((v_WinnerTakeAll_va)AS_GO_HOME).weights[0]  
				= avoidgain;
			((v_WinnerTakeAll_va)AS_GO_HOME).embedded[0] 
				= MS_AVOID_OBSTACLES;

			((v_WinnerTakeAll_va)AS_GO_HOME).weights[1]  
				= noisegain;
			((v_WinnerTakeAll_va)AS_GO_HOME).embedded[1] 
				= MS_NOISE_VECTOR;

			((v_WinnerTakeAll_va)AS_GO_HOME).weights[2]  
				= mtggain;
			((v_WinnerTakeAll_va)AS_GO_HOME).embedded[2] 
				= MS_MOVE_TO_HOMEBASE;
			}
		else
			{
			...
			((v_StaticWeightedSum_va)AS_GO_HOME).weights[0]  
				= avoidgain;
			((v_StaticWeightedSum_va)AS_GO_HOME).embedded[0] 
				= MS_AVOID_OBSTACLES;

			((v_StaticWeightedSum_va)AS_GO_HOME).weights[1]  
				= noisegain;
			((v_StaticWeightedSum_va)AS_GO_HOME).embedded[1] 
				= MS_NOISE_VECTOR;

			((v_StaticWeightedSum_va)AS_GO_HOME).weights[2]  
				= mtggain;
			((v_StaticWeightedSum_va)AS_GO_HOME).embedded[2] 
				= MS_MOVE_TO_HOMEBASE;
			}
	...
	public int takeStep()
		...
		if (pb != null)
			...
			String combo = pb.getoperator();
			if (combo.equals("vector sum"))
				vecsum = true;
			else
				vecsum = false;
			if (vecsum != lastvecsum) configure();
			lastvecsum = vecsum;
			}
	...
	public void trialInit()
		...
		// kill off old ParamBox2 if it exists
		if (pb != null)
			pb.dispose();

		// open a new ParamBox2
		pb = new ParamBox2();
